...
def _shell(bm, uv, zone):
    ...
    h = ZONE_HEIGHT[zone]
    floor_z = rk.room_shell(bm, uv, h, STONE, trim=PLASTER, floor_pigment=MORTAR,
                            door_sides=rk.SIDES)
    return h, floor_z
...
def build_wall_corner(bm, uv):
    ...
    # 2.3 in from the cell edge, not 1.0: the drum's trim course is the
    # widest thing on it (radius * 1.05 == 2.205), so anything closer than
    # that plus a little hangs over the neighbouring cell.
...
def build_drawbridge(bm, uv):
    ...
    # isn't modelled: a deck longer than HALF leaves the 12m cell entirely
    # and lands on top of whatever the generator placed to the south.
    deck_len = 5.0
    mk.paint(bm, mk.add_box(bm, (3.8, deck_len, 0.22), loc=(0, -rk.HALF + deck_len / 2, 0.11)), TIMBER, uv)
    for x in (-1.7, 1.7):
